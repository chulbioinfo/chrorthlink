...
#    GLOBAL_VARIABLE    #
#########################
RefID_list = ["bTaeGut2"]#"hg38", "mOrnAna1", "bTaeGut2", "rGopEvg1", "aRhiBiv1", "sAmbRad1", "fGouWil2"]

target_chr_name = "4" # or All
...
#   Default variables    #
##########################
opt_sorting_chromosome_by_link = True # "chromosome_number" # or # "link"
...
opt_annotation_on = True # annoations on genoPlotR
...
def write_Rscript(RefID, sID_list , iCNT_RefChr):
  ...
  fpout = open(wPATH+"BUSCO_genoPlotR_input/BUSCO_genoPlotR_by_"+RefID+".R",'w')

  tmpline = "makeTransparent = function(..., alpha=0.5) {\n" + "if(alpha<0 | alpha>1) stop("+'"'+"alpha must be between 0 and 1"+'")\n'+ "alpha = floor(255*alpha)\n"+ "newColor = col2rgb(col=unlist(list(...)), alpha=FALSE)\n"+ ".makeTransparent = function(col, alpha) {\n"+"  rgb(red=col[1], green=col[2], blue=col[3], alpha=alpha, maxColorValue=255)\n"+"  }\n"+"  newColor = apply(newColor, 2, .makeTransparent, alpha=alpha)\n"+"return(newColor)\n"+"}\n"
  fpout.write(tmpline)

  tmpline = "library(RColorBrewer)\n"+"qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]\n"+"col_vector = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))\n"
  fpout.write(tmpline)
  ...
  for sID in sID_list[:-1]:
    tmpline = "make_dnaseg_eachID("+'"'+sID+'"'+"),\n"
    fpout.write(tmpline)
  tmpline = "make_dnaseg_eachID("+'"'+sID_list[-1]+'"'+")\n)\nnames(dna_segs)=sID_list\n\n"
  fpout.write(tmpline)

  tmpline = "annotations = list(\n"
  fpout.write(tmpline)

  for sID in sID_list[:-1]:
    tmpline = "make_annotation_eachID("+'"'+sID+'"'+"),\n"
    fpout.write(tmpline)
  tmpline = "make_annotation_eachID("+'"'+sID_list[-1]+'"'+")\n)\n"
  fpout.write(tmpline)

  tmpline = "comparisons = list(\n"
  fpout.write(tmpline)
  ...
  tmpline = 'pdf(paste0(wPATH,'+'"'+'BUSCO_genoPlotR_output/BUSCO_genoPlotR_of_'+'"'+',RefID,'+'"'+'_chr'+target_chr_name+'","'+'.pdf'+'"'+'),'+str(opt_figure_width_size)+','+str(opt_figure_width_size*5/3.0)+')\n'
  fpout.write(tmpline)

  if opt_annotation_on == True:
    tmpline = 'plot_gene_map(dna_segs=dna_segs, comparisons=comparisons, annotations = annotations,main=paste0('+'"'+'Chromosomal orthologous links of '+'"'+',RefID))\ndev.off()\n\n'
  else:
    tmpline = 'plot_gene_map(dna_segs=dna_segs, comparisons=comparisons, main=paste0('+'"'+'Chromosomal orthologous links of '+'"'+',RefID))\ndev.off()\n\n'
  fpout.write(tmpline)
  fpout.close()
...
fpout = open("./4.BUSCO_genoPlotR.R",'w')
for RefID in RefID_list:
  fpin = open(wPATH+"BUSCO_genoPlotR_input/BUSCO_genoPlotR_by_"+RefID+".R",'r')
  for line in fpin:
    fpout.write(line)
  fpin.close()
fpout.close()
